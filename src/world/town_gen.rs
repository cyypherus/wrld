...
impl TownGenerator {
    ...
    pub fn new(seed: u64) -> Self {
        ...
        for _ in 0..50 {
            if rng.gen_bool(0.6) {
                ...
                // Use standalone name
                let root = roots.choose(&mut rng).unwrap();
                let suffix = match rng.gen_range(0..4) {
                    0 => "ton",
                    1 => "ville",
                    2 => "ford",
                    _ => "berg",
    ...
    fn find_town_locations(
        ...
        for y in 3..height - 3 {
            for x in 3..width - 3 {
                ...
                score += self.rng.gen_range(-5.0..5.0);
    ...
    fn build_settlement(
        ...
        let town_name = if !self.town_names.is_empty() {
            let idx = self.rng.gen_range(0..self.town_names.len());
        ...
        let num_buildings = self.rng.gen_range(min_buildings..=max_buildings);
        ...
        for _ in 0..remaining {
            ...
            if let Some(pos) = positions.first() {
                ...
                let owner = if self.rng.gen_bool(0.3) {
    ...
    fn create_road_network(
        ...
        for i in 0..town_centers.len() {
            for j in i + 1..town_centers.len() {
                ...
                if distance < 1000.0 && self.rng.gen_bool(0.3) {
                    edges.push((i, j));
                }
            }
        }
